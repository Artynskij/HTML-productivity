class Slider {
  WRAPPER_SELECTOR = ".slider__wrapper";
  ...
  constructor(selector, config) {
    ...
    const $root =
      typeof selector === "string"
        ...
        : selector;
    this._$root = $root;
    this._$wrapper = $root.querySelector(this.WRAPPER_SELECTOR);
    this._$items = $root.querySelector(this.ITEMS_SELECTOR);
    this._$itemList = $root.querySelectorAll(this.ITEM_SELECTOR);
    this._$controlPrev = $root.querySelector(this.SELECTOR_PREV);
    this._$controlNext = $root.querySelector(this.SELECTOR_NEXT);
    this._$indicatorList = $root.querySelectorAll(this.SELECTOR_INDICATOR);
    ...
    if (this._config.loop) {
      ...
      this._$controlPrev.classList.add(this.CLASS_CONTROL_HIDE);
    }
  ...
  _addEventListener() {
    ...
    function onClick(e) {
      ...
      this._autoplay("stop");
      if ($target.classList.contains(this.CONTROL_CLASS)) {
        e.preventDefault();
  ...
  _setActiveClass() {
    ...
    for (let i = 0, length = $itemList.length; i < length; i++) {
      ...
      if (activeItems.indexOf(index) > -1) {
        $item.classList.add(this.CLASS_ITEM_ACTIVE);
      } else {
        $item.classList.remove(this.CLASS_ITEM_ACTIVE);
      }
    }
  }
  _updateIndicators() {
    ...
    for (let index = 0, length = $itemList.length; index < length; index++) {
      ...
      if ($item.classList.contains(this.CLASS_ITEM_ACTIVE)) {
        $indicatorList[index].classList.add(this.CLASS_INDICATOR_ACTIVE);
      } else {
        $indicatorList[index].classList.remove(this.CLASS_INDICATOR_ACTIVE);
      }
    }
  }
  _move() {
    ...
    if (!this._config.loop) {
      ...
      this._$controlPrev.classList.remove(this.CLASS_CONTROL_HIDE);
      this._$controlNext.classList.remove(this.CLASS_CONTROL_HIDE);
      if (transform === -endTransformValue) {
        this._$controlNext.classList.add(this.CLASS_CONTROL_HIDE);
      } else if (transform === 0) {
        this._$controlPrev.classList.add(this.CLASS_CONTROL_HIDE);
      }
    }
  ...
  _moveTo(index) {
    ...
    for (i = 0, length = $indicatorList.length; i < length; i++) {
      ...
      if ($indicator.classList.contains(this.CLASS_INDICATOR_ACTIVE)) {
  ...
  _refresh() {
    ...
    this._$items.classList.add(this.SLIDER_TRANSITION_OFF);
    ...
    window.requestAnimationFrame(() => {
      this._$items.classList.remove(this.SLIDER_TRANSITION_OFF);
    });
